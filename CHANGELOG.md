...
### Fixed (Critical — unknown-word lattice construction)

- **Unknown-word grouping was Θ(n³) and never consulted `char.def`'s `LENGTH`** (`lattice/mod.rs`): `add_grouped_unknown` emitted a node for *every* prefix of length ≥ 2 of a same-category run, at *every* position with no dictionary hit, deduplicating each candidate with a linear scan of the destination bucket — Θ(n²) nodes found through Θ(n) scans. A 400-character ASCII run took 93 ms, 3,200 characters took 38.4 s and 60,000 characters could not finish at all (an uncatchable trap in the WASM demo). It is replaced by `add_unknown_run_nodes`, which implements MeCab's documented semantics (mecab-0.996 `src/tokenizer.cpp::Tokenizer::lookup`): `GROUP = 1` emits **one** node spanning the whole maximal run, and only while that run is at most `MAX_GROUPING_SIZE + 1 = 25` characters (MeCab's `max-grouping-size` default — verified against mecab 0.996 + IPADIC: 25 `a`s are one token, 26 are two); `LENGTH = n` emits prefix nodes of 1..=n characters and nothing longer. The run scan stops at the first character the semantics cannot use, so lattice construction is linear in the run length.
- **Grouped unknown nodes only ever carried the first `unk.dic` template** (`lattice/mod.rs`): the duplicate scan sat *inside* the per-template loop, so the second and later templates of a category were silently dropped for every grouped node — IPADIC gives ALPHA, KATAKANA, HIRAGANA and KANJI six each. Every template is now emitted, as MeCab's `ADDUNKNWON` macro does, and Viterbi chooses between them: `https`, `info` and `COVID` now come out as 名詞,固有名詞,組織 exactly like mecab, instead of 名詞,一般.
- **Unknown-word prefix nodes are now emitted for non-grouping categories too** (`lattice/mod.rs`): IPADIC's KANJI category is `GROUP = 0 LENGTH = 2`, so the old code — which only ran the run logic when `should_group` was true — could never produce a two-character unknown kanji node. 令和 came out as 令 + 和 where mecab produces 令和.
- Regression coverage: `mecrab/tests/unknown_grouping.rs` (structural node-count budget for 100…60,000 character runs, MeCab semantics fixtures for `GROUP`/`LENGTH`/`max-grouping-size`, and an output-identity corpus of 60 sentences — the three /mecrab playground samples verbatim plus 57 more — gated on `MECRAB_IPADIC_MCAB` pointing at a packed IPADIC blob).

### Fixed (Critical — `kizame dict compile` produced unloadable dictionaries)

- **The compiled output directory could never be loaded** (`kizame/src/commands/dict.rs`, new `kizame/src/commands/dict_compile.rs`): `dict compile` copied `char.def`, `unk.def` and `matrix.def` into the output directory as text, where the readers map exact-size binary images (`CharDef::parse_bytes` accepts only `4 + csize*32 + 0xFFFF*4` bytes; `ConnectionMatrix` only `4 + lsize*rsize*2`; `SysDic` only a file whose magic encodes its own size). The four binary writers mecrab-builder already exported are now wired in: `build_char_bytes`/`pack_char_info` for `char.bin`, `build_matrix_bytes` for `matrix.bin`, `build_unkdic_bytes` for `unk.dic` and `build_sysdic_bytes` for `sys.dic`. Recompiling stock IPADIC now reproduces the `char.bin` and `matrix.bin` that `mecab-dict-index` ships, byte for byte.
- **Source files are decoded, not assumed UTF-8** (`dict_compile.rs`): CSV and `.def` files were read through `BufRead::lines()`, which errors on EUC-JP — the encoding stock IPADIC ships in. All source files now go through `encoding_rs` with the `--charset` encoding, whose default changed from `utf-8` to `euc-jp` to match `mecab-dict-index -f`; a file that does not decode cleanly is an error rather than mojibake in the dictionary. The compiled dictionary always stores UTF-8, so `UTF-8` is what goes into the `sys.dic` / `unk.dic` charset header.
- **`char.def` category names are mapped onto mecrab's `CharCategory` ids** (`dict_compile.rs`): the reader maps a packed `default_type` straight onto that enum, so a category order other than IPADIC's would have silently mis-categorised every character. Names are resolved against the canonical order and an unrepresentable category is a hard error. Unmapped code points receive the mandatory `DEFAULT` category, as MeCab does.
- **Nothing is written until every source has parsed**, so a failed compile no longer leaves a half-built directory behind — and without the `full` feature the command now fails before writing anything instead of after copying the `.def` files.
- Round-trip coverage: `kizame/tests/dict_compile.rs` compiles an EUC-JP source dictionary through the CLI, checks the output is binary-only, loads it with `mecrab::dict::Dictionary::load` and tokenises with it (requires `--features full`).

### Fixed (WASM packaging and feature graph)

- **`--no-default-features --features wasm` did not compile** (`mecrab/Cargo.toml`, `mecrab/src/error.rs`): the `wasm` feature enabled `serde_json` but not `serde`, while `impl From<serde_json::Error> for Error` was gated on `serde` — so `dict::Dictionary::load_with_semantics` failed with E0277 on its `serde_json::from_str(…)?`. The impl is now gated on `serde_json` (the feature it actually needs), and the `wasm` feature lists `serde` alongside `serde_json` like `json` and `python` do.
- **`wasm/pack_dict.js` silently packed source text** (`wasm/pack_dict.js`): missing binary sections fell back to their `.def` source files, and a section missing entirely was written as zero bytes — either way producing a blob that fails inside `loadDictionary()` in the browser. All four sections are now required and validated (dictionary magic/version/type for `sys.dic` and `unk.dic`, exact size formulas for `char.bin` and `matrix.bin`); a `.def`-only directory gets a message naming the file and the command that compiles it, and nothing is written.
