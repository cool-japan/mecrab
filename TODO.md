...
- [x] GPU parity validation (DONE 2026-06-22 — `mecrab-word2vec/tests/gpu_parity.rs`: trains one race-free batch on CPU and GPU from identical deterministic weights and asserts the embeddings match within 1e-4; skips gracefully with no adapter. Ran on the llvmpipe software adapter: max abs diff 7.45e-9. See v0.3.7 section.)
...
## v0.3.7 Additions (2026-06-22)

### Completed

#### CRF Training Optimizers
- [x] **AdaGrad / RMSProp / Adam optimizers** (`viterbi/train.rs`, `train_loop.rs`): per-parameter adaptive learning rates replace plain SGD; `Optimizer` enum + `OptimizerState` (accumulated squared gradients / first+second moments for connection costs and word costs); `DictTrainConfig::optimizer`; `kizame train --optimizer {sgd,adagrad,rmsprop,adam}`. Plain SGD remains the default (byte-identical to prior behaviour).

#### CLI / MeCab Parity
- [x] **`kizame parse -p <file>` partial-parse constraint file** (surface-per-line MeCab `-p` format): each line is `surface` or `surface\tfeature`; surfaces concatenate to the sentence; byte spans become `ForcedSpan` constraints (feature forced when present). Mirrors MeCab `--partial`.

#### GPU
- [x] **GPU parity validation test** (`mecrab-word2vec`): when a real wgpu adapter is present, trains one batch on both CPU and GPU from identical initial weights and asserts the resulting embeddings match within f32 tolerance; gracefully skips (not fails) when no adapter is available in CI. (Ran on the llvmpipe software adapter here: max abs diff 7.45e-9.)
- [x] **Fixed pre-existing temp-file race** in w2v test fixtures (`make_corpus_file`, `write_tiny_corpus`): names now include a process-wide atomic counter + thread id, eliminating the flaky `Io(NotFound)` collisions surfaced under the `gpu` feature.

#### Lattice (Performance)
- [x] **Lattice `nodes_at: Vec<Vec<_>>` → CSR flat layout** (`lattice/mod.rs`): the `Vec<Vec<LatticeNode>>` field is replaced by contiguous CSR storage (`flat: Vec<LatticeNode>` + `offsets: Vec<usize>`), cutting per-lattice heap allocations from O(text_len) tiny vectors to two buffers and giving the Viterbi read phase cache-friendly sequential access. Build + constraint application operate on a mutable bucket form (`build_nodes_at`) then freeze via `from_nodes_at`; the hot path (Viterbi/forward-backward/CRF) reads through the unchanged `nodes_ending_at` accessor + new `positions()` iterator, so output is **bit-identical** — validated by the full suite incl. `segments_sumomo_sentence`, `parallel_build_matches_sequential_sumomo`, exact N-best, and all visualize/constraint tests (648 pass, clippy clean).

### v0.3.8 Additions (2026-06-23)

#### CRF Training (Correctness — the iterative-refit prerequisite)
- [x] **Evolving-cost (iterative) CRF training** (`viterbi/fb.rs`, `train.rs`, `train_loop.rs`): the CRF gradient can now be recomputed under the **current (evolving) model** each epoch instead of always under the immutable system dictionary. Introduced a `pub(crate) trait EdgeCostSource` (connection cost + node word cost) with two implementations: `DictCostSource` (the inference default — byte-identical to the historical hard-coded `dict.connection_cost`/`node.wcost` lookups) and `MatrixCostSource` (reads the in-training `TrainingMatrix` connection costs + accumulated word-cost deltas). `forward_backward`/`compute_edge_expected_counts`/`edge_expected_counts_from_fb`/`compute_sentence_gradient`/`accumulate_batch_gradient` gained generic `_with` variants threading the cost source; the original signatures are retained as thin delegators over `DictCostSource`, so `parse_with_probs`/`score`/LatticeProb and every existing test are **byte-identical**. Opt in via `DictTrainConfig::iterative` (default `false`) / `kizame train --iterative`.
  - **Why this is the real fix:** previously the per-sentence gradient `expected − empirical` was computed from the *initial* dictionary every epoch, so it was *fixed across epochs* — the trainer walked one direction and the per-epoch loss was provably constant (it never reflected the updates). With `--iterative`, expected counts are recomputed under the updated weights, making AdaGrad/RMSProp/Adam (and SGD) **true iterative CRF optimizers**, and unblocking any second-order method.
  - **Validation:** unit tests for the cost-source math; E2E tests on the synthetic dictionary proving (a) `iterative` epoch-0/single-batch is **bit-identical** to non-iterative (pristine matrix ⇒ `MatrixCostSource == DictCostSource`), (b) non-iterative per-epoch loss is exactly flat while iterative loss strictly decreases, (c) the multi-epoch iterative model's word-cost deltas diverge from the fixed-gradient run (the connection matrix may legitimately stay put when its integer-rounded gradient is zero).

#### Batch L-BFGS / OWL-QN optimizer (the second-order method, now unblocked)
- [x] **L-BFGS and OWL-QN for CRF dictionary-cost training** (`viterbi/lbfgs.rs`, `viterbi/train_lbfgs.rs`; `kizame train --optimizer lbfgs [--l1 <C>] [--lbfgs-memory <m>]`). Two layers:
  - **Generic minimizer** (`lbfgs.rs`, dependency-free): Nocedal two-loop recursion with `γ = sᵀy/yᵀy` Hessian scaling, backtracking-Armijo line search, and `sᵀy>0` curvature safeguard; the **OWL-QN** extension (Andrew & Gao 2007) adds the `L1` pseudo-gradient, orthant-aligned direction projection, and orthant-projected line search. `EdgeCostSource` was widened from `i32` to **`f64`** so the objective is *smooth* (exact for the integer sources, so iterative/inference paths stay byte-identical). **Validated independently of the dictionary** on problems with known optima: separable convex quadratic (→ exact minimum), Rosenbrock (→ `(1,1)`), and `L1` soft-threshold (OWL-QN drives `|tᵢ| ≤ C` components to *exactly* `0`).
  - **CRF adapter** (`train_lbfgs.rs`): minimizes the true regularized NLL `f(x) = Σ[cost_gold(x)/T + log Z(x)] + ½·l2·‖x‖²` over the *delta* vector `x` (so `x=0` is the untrained dictionary), with the analytic gradient `(empirical − expected)/T + l2·x` — value and gradient are mutually consistent, so the line search makes real progress. Active parameters are discovered from a probe gradient; lattices are built once and reused. `train_dict` dispatches `Optimizer::Lbfgs` to this batch driver instead of the mini-batch epoch loop. **Validated** E2E on the synthetic dictionary: L-BFGS drives the regularized NLL strictly down from the untrained objective and leaves finite parameters; OWL-QN runs to a finite, non-increasing objective.
  - **Honest scope note:** the optimizer core is *proven correct* and the CRF integration is *proven to reduce the objective*, but end-to-end convergence **quality** on a full real IPADIC corpus is not measured here (no IPADIC in this environment). Mini-batch SGD/AdaGrad/RMSProp/Adam remain the default; L-BFGS is opt-in.

#### Forward-Backward (Correctness — Critical crash fix)
- [x] **Fixed a latent panic in `forward_backward` / `compute_edge_expected_counts` on every real multi-token sentence** (`viterbi/fb.rs`). The public `parse_with_probs` / `score` / `LatticeProb` / BPE-compatible outputs and the WASM/Python prob APIs **panicked** (`index out of bounds`) on any non-trivial input. **Root cause:** the lattice stores a node ending at byte `e` at position `e + 1`, and EOS (`start == end == text_len`) at position `text_len + 1` — the *same* position as the real nodes ending at the last byte. So EOS's predecessor slot resolved to its own (still-being-built) row, and the backward pass left those final nodes' `beta` at the seed value `0`. The bug was never caught because the only "probs" test inspected a hand-built `LatticeProbTable` struct and never ran forward-backward end-to-end. **Fix:** consolidated the duplicated forward/backward into one `forward_backward_tables` helper that (i) reads same-position predecessors from the in-progress row (forward), (ii) seeds position `n − 1` by walking its bucket in reverse so EOS (terminal) is set before the nodes that precede it (backward), and (iii) excludes the spurious EOS→EOS self-edge in the edge-count pass. Dead multi-slot "span" scans (impossible under the one-node-per-end-byte lattice invariant, mirroring the Viterbi forward pass) were removed. **Validation:** new self-contained unit tests build a two-path lattice and assert forward Z (`alpha[EOS]`) == backward Z (`log_z` from BOS) and exact marginals (P=0.5 for each of two equal paths, P=1 for BOS/EOS; biased variant confirms the cheaper path dominates and competing first-segment marginals sum to 1); plus the E2E training tests now exercise it on real synthetic-dictionary sentences.

#### word2vec (Test reliability)
- [x] **Fixed a flaky `model.rs` test under `nextest`** (`mecrab-word2vec`): the test-corpus `temp_path` helper keyed filenames on `nanos` + thread id, but `nextest` runs each test in its own process where the main thread is always `ThreadId(1)`, so two processes at the same nanosecond collided and truncated each other's corpus mid-train → a zero/NaN embedding → intermittent `similarity_self_is_one` failures. Now keyed on process id + a process-local atomic counter (matching the `lib.rs`/`trainer.rs` fixtures); full suite is stable across repeated runs.

### Planned (future rounds)
- [ ] **"5x vs MeCab" benchmark — environment-blocked.** Requires the MeCab C++ binary + a real IPADIC install, neither of which can be provisioned/validated in this environment. Shipping a benchmark harness that asserts a "5x" figure it cannot actually run would be unvalidated scaffolding (no-fabrication policy forbids). Deferred until a machine with MeCab + IPADIC is available.

## Pure Rust Migration (COOLJAPAN Policy)

- [x] **(DONE 2026-06-22, commit b7e1c7d) Replace `flate2` with `oxiarc-deflate` (gzip)** — all 4 streaming `GzDecoder` call sites now use `oxiarc_deflate::GzipStreamDecoder`; `flate2` removed from the workspace (`grep -rn flate2 --include=*.rs --include=*.toml` returns only explanatory comments). Dump ingestion remains constant-memory streaming decompression.

<details><summary>Original task note (kept for provenance)</summary>
